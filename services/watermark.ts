 ...
 *
 * Logos are embedded as base64 to avoid filesystem issues on Vercel serverless.
 */
...
export async function applyWatermark(imageBuffer: Buffer, _mimeType?: string): Promise<Buffer> {
  const meta = await sharp(imageBuffer).metadata();
  const w = meta.width || 800;
  const h = meta.height || 600;

  const variant = await detectLogoVariant(imageBuffer);
  const logoBuf = getLogoBuffer(variant);
